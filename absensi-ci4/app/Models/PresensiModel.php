...
class PresensiModel extends Model
{
    ...
    public function getDataPresensiHarian($tanggal_dari = false, $tanggal_sampai = false, $print = false, $perPage = 10)
    {
        ...
        // CTE sebagai subquery
        $shiftAssignments = $this->db->table('master_shift ms')
        ->select('ms.nama_shift, ms.jam_mulai, ms.jam_selesai, mss.*')
        ->join('master_shift_setting mss', 'mss.id_shift = ms.id')
        ->getCompiledSelect();

        $this->builder = $this->db->table('presensi');
        $this->builder->select('presensi.*, pegawai.nip, pegawai.nama, pegawai.id_lokasi_presensi, lokasi_presensi.nama_lokasi as lokasi_presensi, lokasi_presensi.jam_masuk as jam_masuk_kantor, sa.nama_shift, sa.jam_mulai, sa.jam_selesai');
        $this->builder->join('pegawai', 'pegawai.id = presensi.id_pegawai');
        $this->builder->join('lokasi_presensi', 'lokasi_presensi.id = pegawai.id_lokasi_presensi');
        $this->builder->join("($shiftAssignments) sa", 'sa.id_pegawai = presensi.id_pegawai AND sa.tgl_shift = presensi.tanggal_masuk', 'left');
